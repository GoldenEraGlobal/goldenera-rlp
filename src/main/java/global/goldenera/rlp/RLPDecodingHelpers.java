 ...
 * Neither this class
 * nor any of its method are meant to be exposed publicly, they are too low
 * level.
 */
class RLPDecodingHelpers {
  ...
  /**
   * Read from the provided offset a size of the provided length, assuming this is
   * enough bytes.
   */
  ...
  /**
   * The size of the item payload for a "long" item, given the length in bytes of
   * the said size.
   */
  private static int readLongSize(
    ...
    // We will read sizeLength bytes from item + 1. There must be enough bytes for
    // this or the input
    // is corrupted.
    ...
    // That size (which is at least 1 byte by construction) shouldn't have leading
    // zeros.
