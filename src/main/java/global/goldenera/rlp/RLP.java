...
public abstract class RLP {
  ...
  // As a result, the longest RLP strings will have a prefix composed of 1 byte
  // encoding the type
  ...
  /**
   * Creates a new {@link RLPInput} suitable for decoding the provided RLP encoded
   * value.
   *
   * <p>
   * The created input is strict, in that exceptions will be thrown for any
   * malformed input,
   ...
   * @throws MalformedRLPInputException if {@code encoded} doesn't contain a
   *                                    single RLP encoded item
   *                                    (item that can be a list itself). Note
   *                                    that more deeply nested
   *                                    corruption/malformation of
   *                                    the input will not be detected by this
   *                                    method call, but will be later when the
   *                                    input is
   *                                    read.
   */
  ...
  /**
   * Creates a {@link RLPOutput}, pass it to the provided consumer for writing,
   * and then return the
   * RLP encoded result of that writing.
   *
   * <p>
   * This method is a convenience method that is mostly meant for use with class
   * that have a
   ...
   * }</pre>
   *
   * @param writer A method that given an {@link RLPOutput}, writes some data to
   *               it.
   ...
   *
   * <p>
   * So note in particular that the value is encoded as is (and so not as a scalar
   * in
   * particular).
   *
   ...
   *
   * <p>
   * So note in particular that the value is decoded as is (and so not as a scalar
   * in
   * particular).
   *
   * @param encodedValue The encoded RLP value.
   * @return The single value encoded in {@code encodedValue}.
   * @throws RLPException if {@code encodedValue} is not a valid RLP encoding or
   *                      if it does not
  ...
  public static void validate(final Bytes encodedValue) {
    ...
    while (!in.isDone()) {
      ...
      if (in.nextIsList()) {
        ...
        // Skip does as much validation as can be done in general, without allocating
        // anything.
        in.skipNext();
      }
    }
  ...
  /**
   * Given a {@link Bytes} containing rlp-encoded data, determines the full length
   * of the encoded
