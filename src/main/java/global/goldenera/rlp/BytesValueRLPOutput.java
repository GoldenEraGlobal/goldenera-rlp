...
public class BytesValueRLPOutput implements RLPOutput {
   ...
   *
   * Values written to the output are accumulated in the 'values' list. When a
   * list is started, it
   * is indicated by adding a specific marker in that list (LIST_MARKER).
   * While this is gathered, we also incrementally compute the size of the payload
   * of every list of
   * that output. Those sizes are stored in 'payloadSizes': when all the output
   * has been added,
   * payloadSizes[i] will contain the size of the (encoded) payload of the ith
   * list in 'values'
   * (that is, the list that starts at the ith LIST_MARKER in 'values').
   *
   * With that information gathered, encoded() can write its output in a single
   * walk of 'values':
   * values can be encoded directly, and every time we read a list marker, we use
   * the corresponding
   * payload size to write the proper prefix and continue.
   *
   * The main remaining aspect is how the values of 'payloadSizes' are computed.
   * Computing the size
   * of a list without nesting inside is easy: simply add the encoded size of any
   * newly added value
   * to the running size. The difficulty is with nesting: when we start a new
   * list, we need to
   * track both the sizes of the previous list and the new one. To deal with that,
   * we use the small
   * stack 'parentListStack': it stores the index in 'payloadSizes' of every
   * currently "open" lists.
   * In other words, payloadSizes[parentListStack[stackSize - 1]] corresponds to
   * the size of the
   * current list, the one to which newly added value are currently written (until
   * the next call
   * to 'endList()' that is, while payloadSizes[parentListStack[stackSize - 2]]
   * would be the size
   * of the parent list, ....
   *
   * Note that when a new value is added, we add its size only the currently
   * running list. We should
   * add that size to that of any parent list as well, but we do so indirectly
   * when a list is
   * finished: when 'endList()' is called, we add the size of the full list we
   * just finished (and
   * whose size we have now completed) to its parent size.
   *
   * Side-note: this class internally and informally use "element" to refer to a
   * non list items.
   */
  ...
  // For every value i in values, rlpEncoded.get(i) will be true only if the value
  // stored is an
  // already encoded item.
  private final BitSet rlpEncoded = new BitSet();

  // First element is the total size of everything (the encoding may be a single
  // non-list item, so
  // this handles that case more easily; we need that value to size out final
  // output). Following
  ...
  public void startList() {
    ...
    // The new current list size is store in the slot we just made room for by
    // incrementing
    // listsCount
  ...
  public void endList() {
    ...
    // We just finished an item of our parent list, add it to that parent list size
    // now.
   ...
   * @throws IllegalStateException if some opened list haven't been closed (the
   *                               output is not valid
   *                               as is).
   */
  ...
  public void writeEncoded(final MutableBytes mutableBytes) {
    // Special case where we encode only a single non-list item (note that
    // listsCount is initially
    // set to 1, so listsCount == 1 really mean no list explicitly added to the
    // output).
    if (listsCount == 1) {
