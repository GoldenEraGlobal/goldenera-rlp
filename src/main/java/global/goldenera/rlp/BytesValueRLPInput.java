...
public class BytesValueRLPInput implements RLPInput {
  ...
  private long currentItem; // Offset in value to the beginning of the item (or value.size() if done)
  ...
  // and endOfListOffset
  // holds the offset in value at which each list ends (indexed by depth). Allows
  // to know if we're
  ...
  private void init(final long inputSize, final boolean shouldFitInputSizeExactly) {
    ...
    currentItem = 0;
    // Initially set the size to the input as prepareCurrentItem() needs it. Once
    // we've prepared the
    // top level item, we know where that item ends exactly and can update the size
    // to that more
    // precise value (which basically mean we'll throw errors on malformed inputs
    // potentially
    // sooner).
    size = inputSize;
    prepareCurrentItem();
    ...
    // No matter what, if the first item advertise a payload ending after the end of
    // the input, that
    // input is corrupted.
    if (size > inputSize) {
      // Our error message include a snippet of the input and that code assume size is
      // not set
      // outside the input, and that's exactly the case we're testing, so resetting
      // the size
  ...
  private void setTo(final long item) {
    currentItem = item;
    if (currentItem >= size) {
      // Setting somewhat safe values so that multiple calls to setTo(nextItem())
      // don't do anything
  ...
  private void prepareCurrentItem() {
    ...
    // payload.
    try {
      final RLPDecodingHelpers.RLPElementMetadata elementMetadata = RLPDecodingHelpers
          .rlpElementMetadata(this::inputByte, size, currentItem);
      ...
      final String message = String.format(exception.getMessage() + errorMessageSuffix, getErrorMessageSuffixParams());
      throw new RLPException(message, exception);
    }
  }

  private void validateCurrentItem() {
    // Validate that a single byte SHORT_ELEMENT payload is not <= 0x7F. If it is,
    // it should have
  ...
  public boolean isDone() {
    // The input is done if we're out of input, but also if we've called leaveList()
    // an appropriate
    // amount of times.
  ...
  public InetAddress readInetAddress() {
    ...
    try {
      ...
      // InetAddress.getByAddress() only throws for an address of illegal length, and
      // we have
  ...
  /**
   * Enters the list, but does not return the number of item of the entered list.
   * This prevents
  ...
  public int enterList(final boolean skipCount) {
    ...
    // The first list element is the beginning of the payload. Its end is the end of
    // this item.
